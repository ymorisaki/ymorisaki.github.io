...
/* harmony import */ var _match_height__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./match-height */ "./src/js/match-height.js");



Object(_match_height__WEBPACK_IMPORTED_MODULE_2__["matchHeight"])();
...
function matchHeight() {
  ...
  function setMatchHeight(rootEl, options) {
    ...
    return rootElement.forEach(function (root) {
      /**
       * 高さ揃え機能
       * @constructor
       */
      var MatchHeight = function MatchHeight() {
        this.root = root;
        this.matchHeightItem = Array.prototype.slice.call(this.root.querySelectorAll(this.root.getAttribute('data-align-target')));
        this.sameHeighList = null;
      };
      ...
      MatchHeight.prototype = {
        ...
        /**
         * 配置Y軸が同一にある要素を格納する二次元配列を定義
         * @return {void}
         */
        setTwoDimeAry: function setTwoDimeAry() {
          var _this = this;

          var pageYAry = []; // 本来不要な非同期処理、jsとcssが一体化している場合のみ仮に使用している

          setTimeout(function () {
            _this.matchHeightItem.forEach(function (item) {
              var style = window.getComputedStyle(item);
              var marginTop = parseInt(style.marginTop.replace(/px/, ''));
              var paddingTop = parseInt(style.paddingTop.replace(/px/, ''));
              var itemYVal = item.getBoundingClientRect().top + marginTop + paddingTop;
              pageYAry.push(itemYVal);
              item.setAttribute('data-top', itemYVal);
            });
          }, 1);
          console.log(this.matchHeightItem);
        }
      };
      var matchHeight = new MatchHeight();
      matchHeight.init();
    });
  }
}
